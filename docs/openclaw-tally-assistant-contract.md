...
Confirming a budget happens in the unified Review inbox: accepting a `monthly_budget_proposal` (pending, or `answered` after an approve reply) writes a confirmed `monthly_budgets` row for the month, supersedes any prior confirmed budget for that month, and records `monthly_budget.confirmed`/`monthly_budget.superseded` audit events. Confirmed budgets are readable but not writable from browser sessions; only the accept flow writes them. Once a month has a confirmed budget, current-month guardrails and `budget_threshold` messages prefer the confirmed category amounts over historical averages, and the compiler stops proposing for that month.
