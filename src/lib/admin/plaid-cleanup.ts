...
async function updateNullByIds(
  client: AdminClient,
  table: keyof Database["public"]["Tables"],
  userId: string,
  columnToNull: string,
  ...
  const tableClient = client.from(table as never) as {
  ...
  const result = await tableClient
    .update({ [columnToNull]: null })
    .eq("user_id", userId)
    .in(matchColumn, [...values]);

  if (result.error) throw new Error(`Clear ${table}.${columnToNull}: ${result.error.message}`);
}
...
export async function executePlaidItemCleanup(client: AdminClient, options: PlaidCleanupOptions) {
  ...
  await updateNullByIds(client, "reimbursement_records", options.userId, "received_transaction_id", "received_transaction_id", ids.enrichedTransactionIds);
  await updateNullByIds(client, "reimbursement_records", options.userId, "split_id", "split_id", ids.transactionSplitIds);
  await updateNullByIds(client, "recurring_expenses", options.userId, "account_id", "account_id", ids.accountIds);
  await updateNullByIds(client, "recurring_expenses", options.userId, "last_transaction_id", "last_transaction_id", ids.enrichedTransactionIds);

  if (ids.plaidItemIds.length > 0) {
    const result = await client
      .from("plaid_items")
      .delete()
      .eq("user_id", options.userId)
      .in("id", ids.plaidItemIds);

    if (result.error) throw new Error(`Delete Plaid items: ${result.error.message}`);
  }
