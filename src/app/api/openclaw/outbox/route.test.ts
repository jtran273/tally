...
const PATH = "/api/openclaw/outbox";

test.beforeEach(saveOpenClawEnv);
test.afterEach(restoreOpenClawEnv);

test("OpenClaw outbox rejects unauthorized callers", async () => {
  configureOpenClawServer();
  const response = await GET(openClawRequest(PATH, { token: "wrong-token" }));
  assert.equal(response.status, 401);
});

test("OpenClaw outbox rejects an invalid limit", async () => {
  configureOpenClawServer();
  const response = await GET(openClawRequest(`${PATH}?limit=abc`, { token: OPENCLAW_TEST_TOKEN }));
  assert.equal(response.status, 400);
});

test("OpenClaw outbox rejects an invalid min_priority", async () => {
  configureOpenClawServer();
  const response = await GET(openClawRequest(`${PATH}?min_priority=urgent`, { token: OPENCLAW_TEST_TOKEN }));
  assert.equal(response.status, 400);
});

test("OpenClaw outbox reports missing server configuration", async () => {
  configureOpenClawTokenOnly();
